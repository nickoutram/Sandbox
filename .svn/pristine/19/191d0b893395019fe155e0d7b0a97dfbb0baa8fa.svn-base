...
figure(1), clf, X = linspace(0,200,1000);
for k_f = 5:20
    LL_Crosssection(k_f) = sub_Likelihood(k_f,NumReefs,D,Pop_dens,reef_size,SampledReefs_Adults, ...
...
subplot(2,1,1); plot(nonzeros(LL_Crosssection))
...
for i = 1:length(SampledReefs_Juveniles)
    ...
    ParentageMatrix_ThisRow = ParentageMatrix(ThisJuvReef,:);
    ParentageMatrix_ThisRow(isnan(ParentageMatrix_ThisRow)==1) = [];

    % Calculate the number of samples for this reef by the number of non-NAN entries
    SampleSize_Juveniles_ThisReef = length(ParentageMatrix_ThisRow);
